...
, isRelease ? false, withTools ? true, withChainloading ? false
, customLogo ? null, }:
...
let
  ...
  stdenvOpts = {
    ...
    targetPlatform.rust.rustcTarget =
      "${stdenv.hostPlatform.parsed.cpu.name}-unknown-none-softfloat";
    targetPlatform.rust.rustcTargetSpec =
      "${stdenv.hostPlatform.parsed.cpu.name}-unknown-none-softfloat";
  };
...
in stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ dtc ] ++ lib.optionals withChainloading [
    rustPackages.rustc
    rustPackages.cargo
